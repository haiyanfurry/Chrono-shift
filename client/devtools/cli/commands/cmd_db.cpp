...
static int cmd_db(int argc, char** argv)
{
    if (argc < 1) {
        std::println("用法:");
        std::println("  db list users                  - 列出所有用户");
        std::println("  db list messages               - 列出消息数据");
        std::println("  db list friends                - 列出好友关系");
        std::println("  db list templates              - 列出所有模板");
        return -1;
    }
    ...
    if (subcmd == "list") {
        if (argc < 2) {
            std::println(stderr, "用法: db list <type> (users/messages/friends/templates)");
            return -1;
        }
        ...
        if (type == "users") {
            ...
            std::println(stderr, "未知类型: {} (可用: users, messages, friends, templates)", type);
            return -1;
        }

        std::println("[*] 查询数据库: {}", path);
        ...
        std::println(stderr, "未知 db 子命令: {}", subcmd);
        return -1;
    }
}
