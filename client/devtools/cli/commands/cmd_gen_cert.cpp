...
static std::string get_openssl_cmd() noexcept
{
    ...
    if (check_msys2_openssl()) {
        cli::println("[*] 使用 MSYS2 OpenSSL: {}", MSYS2_OPENSSL_PATH);
        return std::string("\"") + MSYS2_OPENSSL_PATH + "\"";
    }
#endif
    /* 回退到系统 PATH */
    cli::println("[*] 使用系统 PATH 中的 openssl");
    return "openssl";
}
...
static int cmd_gen_cert(int argc, char** argv)
{
    ...
    cli::println("{}=== 快速自签名证书生成 ==={}",
                 cli::COLOR_BOLD, cli::COLOR_RESET);
    cli::println("  输出目录: {}", output_dir);
    cli::println("  Common Name: {}", cn);
    cli::println("  有效天数: {}", days);

    /* 确保输出目录存在 */
    if (!ensure_dir(output_dir)) {
        cli::println(stderr, "{}[-] 无法创建输出目录: {}{}",
     ...
     * 步骤 2: 生成自签名证书
     * ======================================================== */
    cli::println("[2/3] 生成自签名证书 (SHA-256)...");
    {
     ...
     * ======================================================== */
    cli::println("");
    cli::println("{}[+] 证书生成完成!{}", cli::COLOR_GREEN, cli::COLOR_RESET);
    cli::println("  {}证书: {}{}", cli::COLOR_CYAN, cert_path, cli::COLOR_RESET);
    cli::println("  {}私钥: {}{}", cli::COLOR_CYAN, key_path, cli::COLOR_RESET);
    cli::println("  {}Common Name: {}{}", cli::COLOR_CYAN, cn, cli::COLOR_RESET);
    cli::println("  {}有效期: {} 天{}", cli::COLOR_CYAN, days, cli::COLOR_RESET);
    cli::println("");
    cli::println("  使用方式:");
    cli::println("    ClientHttpServer::set_tls_cert_paths(\"{}\", \"{}\");",
                 cert_path, key_path);
    cli::println("    ClientHttpServer::set_use_https(true);");
    cli::println("");
    cli::println("  {}注意: 此为自签名证书，仅用于开发/调试环境。{}",
                 cli::COLOR_YELLOW, cli::COLOR_RESET);
    cli::println("  {}      生产环境请使用受信任 CA 签发的证书。{}",
                 cli::COLOR_YELLOW, cli::COLOR_RESET);
