...
static int cmd_crypto(int argc, char** argv)
{
    ...
    if (subcmd == "test") {
        ...
        const char* plaintext = "Hello, Chrono-shift! 这是一�?E2E 加密测试消息�?;
        ...
        cli::println("  或直接使�?OpenSSL EVP �?(本测�?");
        return 0;

    } else {
        cli::println(stderr, "未知 crypto 子命�? {}", subcmd);
        return -1;
    }
}
...
static int cmd_crypto(int /*argc*/, char** /*argv*/)
{
    cli::println(stderr, "[-] crypto 命令需�?OpenSSL 支持 (HTTPS_SUPPORT=0)");
    return -1;
}
#endif  /* HTTPS_SUPPORT */
